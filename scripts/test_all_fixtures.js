...
const doc1 = [
  ...
  'Executive is eligible for an annual performance bonus of up to 25% of base salary based on milestones.',
...
const doc5 = [
  ...
  'Vendor guarantees 99.9% monthly platform uptime excluding scheduled maintenance.',
  'Customer is entitled to a 10% monthly service credit if monthly uptime drops below 99.9% in any calendar month.',
...
(async () => {
  for (const f of files) {
    ...
    try {
      ...
      console.log(`[PASS] ${f.name} -> ${parsed.text.length} chars | First line: "${parsed.text.trim().split('\n')[0]}"`);
    } catch(e) {
