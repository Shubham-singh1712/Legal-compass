 ...
 * fixtureTexts.ts
 *
 * Exact raw text of the 6 verified test fixture documents.
 * Enables instant, reliable analysis in both client and server environments
 * without requiring file system access or re-parsing.
 */
...
export const FIXTURE_DOCUMENTS: Record<string, FixtureData> = {
  employment: {
    id: 'employment',
...
This Agreement is governed by the laws of the State of New York.`,
  ...
  saas: {
    ...
    pageCount: 1,
    text: `ENTERPRISE SAAS VENDOR AGREEMENT
...
This Agreement shall be governed by the laws of the State of Delaware.`,
  ...
  agency: {
    ...
    pageCount: 1,
    text: `CREATIVE AGENCY MASTER SERVICES AGREEMENT
...
/**
 * Resolves a documentId or filename into a matching FixtureData if available.
 */
export function resolveFixture(idOrName: string): FixtureData | undefined {
  if (!idOrName) return undefined;
  const lower = idOrName.toLowerCase().trim();

  // Direct alias match
  if (FIXTURE_DOCUMENTS[lower]) return FIXTURE_DOCUMENTS[lower];

  // Filename or partial slug match
  for (const fixture of Object.values(FIXTURE_DOCUMENTS)) {
    if (lower === fixture.filename.toLowerCase()) return fixture;
    if (lower.includes(fixture.id)) return fixture;
    const cleanFile = fixture.filename.toLowerCase().replace(/\.pdf$/, '').replace(/[^a-z0-9]/g, '');
    const cleanInput = lower.replace(/\.pdf$/, '').replace(/[^a-z0-9]/g, '');
    if (cleanInput.includes(cleanFile) || cleanFile.includes(cleanInput)) return fixture;
  }
